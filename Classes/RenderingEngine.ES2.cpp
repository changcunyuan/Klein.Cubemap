...
void RenderingEngine::Initialize()
{
    ...
    glGenTextures(1, &m_textures.Cubemap);
    glBindTexture(GL_TEXTURE_CUBE_MAP, m_textures.Cubemap);
    for (int f = 0; f < 6; ++f) {
        GLenum format = GL_RGB;
        GLenum type = GL_UNSIGNED_SHORT_5_6_5;
        GLenum face = GL_TEXTURE_CUBE_MAP_POSITIVE_X + f;
        PVRTextureHeader* header = (PVRTextureHeader*) faces[f];
        GLsizei w = header->Width;
        GLsizei h = header->Height;
        const unsigned int* texels = faces[f] + header->HeaderSize / 4;
        glTexImage2D(face, 0, format, w, h, 0, format, type, texels);
    }
